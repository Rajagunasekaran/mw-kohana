...
class Model_Admin_imagelibrary extends Model
{
	public function getParentImgFolder(){
		if(Helper_Common::is_admin()){
			$pfoldsql = "SELECT * FROM `img_folders` WHERE folder_status=0 AND sub_folder_level='0' ORDER BY folder_id ASC";
		}else{
	...
	public function getSubImgFolder($fid=0){
		$sfoldsql = "SELECT * FROM `img_folders` WHERE folder_status=0 AND sub_folder_level='1' ".(($fid =='2' || $fid =='6') ? ' AND folder_id !=4' : '')." ORDER BY folder_id ASC";
	...
	public function getFolderImages($sfid=0, $fid=0, $siteid='', $slimit=0, $elimit=10){
		$userid = Auth::instance()->get_user()->pk();
		$fid    = (($fid==0 || $fid=='') ? '0' : $fid);
		$subfid = (($sfid==0 || $sfid=='') && $fid=='1' ? '4,5' : (($sfid==0 || $sfid=='') && !empty($fid) ? '5' : $sfid));
		$site_tabl = $cond = '';
	...
	public function get_Imageslistbyfilter($filterval){
		...
		if($fid == 0  && $subfid == 0){
			$sql = "SELECT count(*) FROM `img` img LEFT JOIN `img_tags` it ON img.img_id=it.img_id LEFT JOIN `tag` t ON it.tag_id=t.tag_id JOIN sites s ON (img.site_id=s.id) LEFT JOIN default_site_mod_map as ds ON (ds.record_id = img.img_id AND ds.record_type_id=3 AND ds.site_id in ($siteid)) WHERE img.status_id=1 AND img.default_status=1 AND (ds.record_mod_action!=2 OR ds.id is NULL) AND s.is_active = 1 AND s.is_deleted = 0 ".$f_start." ".$f." ".$f_end." GROUP BY img.img_id";
			$rescnt = DB::query(Database::SELECT,$sql)->execute()->as_array();

			$itemsql = "SELECT img.*, 'Default Images' as folder_title FROM `img` img LEFT JOIN `img_tags` it ON img.img_id=it.img_id LEFT JOIN `tag` t ON it.tag_id=t.tag_id JOIN sites s ON (img.site_id=s.id) LEFT JOIN default_site_mod_map as ds ON (ds.record_id = img.img_id AND ds.record_type_id=3 AND ds.site_id in ($siteid)) WHERE img.status_id=1 AND img.default_status=1 AND (ds.record_mod_action!=2 OR ds.id is NULL) AND s.is_active = 1 AND s.is_deleted = 0 ".$f_start." ".$f." ".$f_end." GROUP BY img.img_id ORDER BY ".$orderBy.", img.img_id DESC limit ".$slimit.", ".$elimit;
			$query = DB::query(Database::SELECT,$itemsql);
			$itemlist = $query->execute()->as_array();
			$imgfiles = array();
			if($itemlist!=null && count($itemlist)>0){
	...
	public function InsertImg($imgtitle, $imgurl, $fid, $subfid, $siteid, $default_status=0, $copyflag=0){
		...
		if(!empty($imgurl)){
			$sqlimg = "INSERT INTO `img` (img_title, img_type, img_url, subfolder_id, parentfolder_id, access_id, user_id, site_id, default_status, date_created, date_modified) VALUES ('".$this->escapeStr($imgtitle)."', ".($subfid!=0 ? $imgtypeid : '2').", '".trim($imgurl)."', $subfid, ".(!empty($fid) ? $fid : "'0'").", $useraccess, $userid, $siteid, $default_status, '$datetime', '$datetime')";
			...
			if($copyflag==0){
				$itemsql = "SELECT img.*, ifo.folder_title FROM `img` img LEFT JOIN `img_folders` ifo ON ifo.folder_id=img.parentfolder_id WHERE img.status_id=1 AND img.site_id=".$siteid." AND img.subfolder_id=".$subfid." AND img.parentfolder_id=".$fid." AND img.img_id=".$imgnewid." ORDER BY img.date_created DESC";
				$query = DB::query(Database::SELECT,$itemsql);
				$itemlist = $query->execute()->as_array();
				return $itemlist;
			}
	...
	public function ReplaceImg($imgtitle, $imgurl, $fid, $subfid, $imgid){
		...
		$itemsql = "SELECT img.*, ifo.folder_title FROM `img` img LEFT JOIN `img_folders` ifo ON ifo.folder_id=img.parentfolder_id WHERE img.status_id=1 AND img.site_id=".$siteid." AND img.subfolder_id=".$subfid." AND img.parentfolder_id=".$fid." AND img.img_id=".$imgid." ORDER BY img.img_id DESC";
		$query = DB::query(Database::SELECT,$itemsql);
		$itemlist = $query->execute()->as_array();
		return $itemlist;
	}
	...
	public function doCopyAndDuplicateImages($postdata){
		...
		if(isset($postdata['duplicateimg'])){
			$targetId = $postdata['duplicateimg'];
		}elseif(isset($postdata['copyimg'])){
			$targetId = $postdata['copyimg'];
		}else{
			return false;
		}
		// print_r($imgrow);echo $targetId;exit;
		/*copy img*/
		if(!empty($imgrow) && count($imgrow) > 0){
			if(!empty($targetId)){
				$copyimgid = $this->InsertImg($imgrow[0]['img_title'], $imgrow[0]['img_url'], $targetId, $imgrow[0]['subfolder_id'], $siteid, ($targetId == '6' ? '1' : '0'), 1);
			}
			if(!empty($copyimgid)){
