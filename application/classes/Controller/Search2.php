...
class Controller_Search1 extends Controller {
	...
	public function action_getmodelTemplate(){
		...
		$addOptions		= Arr::get($_GET, 'addOptions'); //''=>false,true =>true;
		$requestFrom	= Arr::get($_GET, 'requestFrom'); //''=>false,true =>true;
		if(!empty($title))
		...
		if(!empty($action) && trim($action) == 'workoutFolder'){
			$response = '<div class="modal-dialog modal-md">
						...
						<div class="modal-header">
							<div class="row">
								<div class="title-header">
									<div class="col-xs-3">
										<a data-role="none" data-ajax="false" href="javascript:void(0);" class="triangle confirm pointers">
									...
									<div class="col-xs-3 save-icon-button">
										<button data-role="none" data-ajax="false" class="update_folder btn btn-default '.(isset($fid) && is_numeric($fid) && $method !='editfolder' && !empty($fid) ? ' datacol' : ' activedatacol').'" name="f_method" '.(isset($fid) && is_numeric($fid) && !empty($fid) ? 'onclick="return changeFolderName();"' : '').' type="submit">ok</button>
									</div>
								</div>
							</div>
						</div>
	...
	if(isset($fid) && is_numeric($fid) && !empty($fid)){
		...
		if(isset($result) && !empty($result) && count($result) > 0){
			$response .= 			'<input type="text" id="folder_name" onkeypress="enableSavebutton();" name="folder_name" required="true" class="form-control input-sm" value="'.$result['folder_title'].'" /><input type="hidden" name="old_folder_name" class="form-control input-sm" value="'.$result['folder_title'].'" /><input type="hidden" name="f_id" class="form-control input-sm" value="'.$result['id'].'" />';
		}else{
	...
	$response .=					'<input type="hidden" name="f_foldid" class="form-control input-sm" value="'.$foldid.'" />
									<input type="hidden" name="f_method" class="form-control input-sm" value="'.$method.'" />
									</div>
	...
	$response .=					'</div>
				...
				$response .='<div class="modal-footer">
						   ...
						   <button data-role="none" data-ajax="false" class="btn btn-default update_folder'.(isset($fid) && $method !='editfolder' && is_numeric($fid) && !empty($fid) ? ' datacol' : ' activedatacol').'" name="f_method" '.(isset($fid) && is_numeric($fid) && !empty($fid) ? 'onclick="return changeFolderName();"' : '').' type="submit">ok</button>
						</div>
						</form>
						</div>
					</div>';
			...
			$response = '<div class="modal-dialog modal-md">
							...
							<div class="modal-header">
								<div class="row">
									<div class="title-header">
										<div class="col-xs-2">
											<a data-role="none" data-ajax="false" data-text="Clicking BACK or CLOSE will discard any changes. Clicking INSERT will apply any content changes. Continue with exiting?" href="javascript:void(0);" class="triangle confirm pointers">
												<i class="fa fa-caret-left iconsize"></i>
											</a>
										</div>
										<div class="col-xs-6">
				...
				$response .= '</div><div class="modal-footer"><button data-role="none" data-ajax="false" type="button" class="btn btn-default confirm pointers" data-text="Clicking BACK or CLOSE will discard any changes. Clicking INSERT will apply any content changes. Continue with exiting?" style="margin-right:20px;">close</button><button data-role="none" data-ajax="false" type="button" class="btn btn-default activedatacol" onclick="insertInputdata();">insert</button></div></form></div></div>';
			...
			if(empty($modelType))
				$modelType = 'myModal';
			$response = $XRciselib.'<div class="modal-dialog"><div class="modal-content">
					...
					if($method=='title'){
						...
						$response .= '<div class="row"><div class="mobpadding"><div class="border borderbottom"><div class="col-xs-2 aligncenter"><i id="incIcon" class="fa fa-chevron-up iconsize pointers" onclick="increment('."'exercise_resistance'".');"></i></div><div class="col-xs-6"><input type="type" readonly="readonly" step="any" min="0" value="'.(isset($exerciseRecord['goal_resist']) && !empty($exerciseRecord['goal_resist']) ? $exerciseRecord['goal_resist'] : '').'" id="exercise_resistance" name="exercise_resistance" required="true" class="onlynumber numeric form-control input-lg" style="width:100%"/></div><div class="col-xs-2 aligncenter"><i id="decIcon" class="fa fa-chevron-down iconsize pointers" onclick="decrement('."'exercise_resistance'".');"></i></div>';
						...
						$response .= '<div class="row"><div class="mobpadding"><div class="border borderbottom"><div class="col-xs-2 aligncenter"><i id="incIcon" class="fa fa-chevron-up iconsize pointers" onclick="increment('."'exercise_distance'".');"></i></div><div class="col-xs-6"><input type="type" step="any" min="0" value="'.(isset($exerciseRecord['goal_dist']) && !empty($exerciseRecord['goal_dist']) ? $exerciseRecord['goal_dist'] : '').'" id="exercise_distance" name="exercise_distance" readonly="readonly" required="true" class="form-control onlynumber numeric input-lg" style="width:100%"/></div><div class="col-xs-2 aligncenter"><i id="decIcon" class="fa fa-chevron-down iconsize pointers" onclick="decrement('."'exercise_distance'".');"></i></div><div class="col-xs-2 aligncenter"><a data-role="none" data-ajax="false" href="javascript:void(0)" onclick="clearInputField('."'exercise_distance'".');"><i class="fa fa-times"></i></a></div></div>';
						...
						$response .= '<div class="row"><div class="mobpadding"><div class="border borderbottom"><div class="col-xs-2 aligncenter"><i id="incIcon" class="fa fa-chevron-up iconsize pointers" onclick="increment('."'exercise_rate'".');"></i></div><div class="col-xs-6"><input type="type" readonly="readonly" step="any" min="0" value="'.(isset($exerciseRecord['goal_rate']) && !empty($exerciseRecord['goal_rate']) ? $exerciseRecord['goal_rate'] : '').'" id="exercise_rate" name="exercise_rate" required="true" class="form-control onlynumber numeric input-lg" style="width:100%"/></div><div class="col-xs-2 aligncenter"><i id="decIcon" class="fa fa-chevron-down iconsize pointers" onclick="decrement('."'exercise_rate'".');"></i></div><div class="col-xs-2 aligncenter"><a data-role="none" data-ajax="false" href="javascript:void(0)" onclick="clearInputField('."'exercise_rate'".');"><i class="fa fa-times"></i></a></div></div>';
				...
				<button style="margin-right: 20px;" data-role="none" data-ajax="false" type="button" class="btn btn-default" onclick="closeModelwindow('."'".$modelType."'".');">close</button><button data-role="none" data-ajax="false" class="btn" onclick="insertExtraToParentHidden('."'".$modelType."'".');" name="f_method" style="background-color:#fff" type="button"><i class="fa fa-sign-in" style="font-size:30px;" ></i></button></div></form></div></div>';
					...
					$('.time-picker1').datetimepicker({ format:'HH:mm:ss' });$('.time-picker2').datetimepicker({ format:'mm:ss' }); ".(!empty($exerciseRecord['goal_unit_id']) ? " if($('#exerciselib')) { $('#exerciselib').bootstrapSwitch('state', true); $('#exerciselib-model').modal(); 
						...
						")." if($('#exerciselib')){
								...
								$('#exerciselib').on('switchChange.bootstrapSwitch', function (event, state) {
							if(state && ($('#exercise_unit').val() == '0' || $('#exercise_unit').val() == '')) {  $('#exerciselib-model').modal();resetWhileOpen(); 
			...
			$response = '<div class="modal-dialog modal-md">
					...
					$response .= '</div><div class="modal-footer"><button data-role="none" data-ajax="false" type="button" class="btn btn-default" data-dismiss="modal">close</button></div></form></div></div>';
			...
			$response = '<div id="preview-exercise" class="modal-dialog xrunitdata-modal">
							<div class="modal-content">
							<div class="modal-header">
								<div class="row">
									<div class="popup-title">
				...
				$response .= '<div class="modal-footer"><button data-role="none" data-ajax="false" type="button" class="btn btn-default" data-dismiss="modal">close</button>'.($editFlag ? '<button type="button" class="btn btn-default allow-edit">More</button>' : '').'</div></div></div>'.($editFlag ? '<div data-backdrop="static" data-keyboard="false" role="dialog" class="modal fade" id="confirm"><div class="modal-dialog xrunitdata-modal"><div class="modal-content"><div class="modal-body">Would you like to switch to edit-mode to modify this '.(!empty($logId) ? 'Workout Journal?' : (!empty($assignId) ? 'Assigned Plan?' : 'Workout Record?')).'</div><div class="modal-footer"><button type="button" class="btn" onclick="$('."'#confirm'".').modal('."'hide'".');">cancel</button><a data-ajax="false" data-role="none" href="'.URL::base(TRUE).'exercise/'.(!empty($logId) ? 'workoutlog/' : (!empty($assignId) ? 'assignedplan/' : 'workoutrecord/')).(!empty($logId) ? $logId : (!empty($assignId) ? $assignId : $foldid)).'/?act=edit&edit='.$fid.'" class="btn btn-primary">Yes</a></div></div></div></div>' : '');
			...
			$response 	= '<div class="modal-dialog xrunitdata-modal">
							<div class="modal-content">
								<div class="modal-header">
									<div class="row">
										<div class="popup-title">
									...
									if(!empty($type) && (strtolower($type) == 'assigned' || strtolower($type) == 'logged')){
										...
										$response .= '<i class="fa fa-caret-left iconsize"></i>
											...
											<div class="col-xs-3 save-icon-button activedatacol">
												<button class="btn btn-default activedatacol '.(!empty($type) && (strtolower($type) == 'myexercise' || strtolower($type) == 'sampleexercise' || strtolower($type) == 'sharedexercise') ? '" onclick="getXrsetOptionsPopup('."'".$fid."','".$type."','".(file_exists($exerciseRecord['img_url']) ? URL::base().$exerciseRecord['img_url'] : '')."','".addslashes(ucfirst($exerciseRecord['title']))."'".')"' : 'hide"').' data-role="none" data-ajax="false" type="button">next</button>
											</div>
										</div>
									</div>
								</div>
							...
							$equipparam = (!empty($exerciseRecord['muscle_title']) ? ucfirst($exerciseRecord['muscle_title']) : '').', ';
							...
							$response	.='<div class="row">
									...
									if(!empty($type) && (strtolower($type) == 'assigned' || strtolower($type) == 'logged')){
										...
										$response .= 'close</a>'.(!empty($type) && (strtolower($type) == 'myexercise' || strtolower($type) == 'sampleexercise' || strtolower($type) == 'sharedexercise') ? '<button class="btn btn-default activedatacol '.(!empty($type) && (strtolower($type) == 'myexercise' || strtolower($type) == 'sampleexercise' || strtolower($type) == 'sharedexercise') ? '" onclick="getXrsetOptionsPopup('."'".$fid."','".$type."','".(file_exists($exerciseRecord['img_url']) ? URL::base().$exerciseRecord['img_url'] : '')."','".addslashes(ucfirst($exerciseRecord['title']))."'".')"' : 'hide"').' data-role="none" data-ajax="false" type="button">next</button>' : '').'</div></div></div>';
					$response .= '</div>';
			...
			$response = '<div class="modal-dialog modal-md">
							...
							if(!empty($exerciseRecord['goal_unit_id'])){
								...
								$response .= '</div>
									...
									<div class="row">
										...
										$response .= $parameter.'</span>
														<input type="hidden" value="'.$paratime.'" class="time-picker exercise_time_hidden" name="exercise_time_hidden"/>
													</div>
												</div>
					...
					$response .= '<div class="modal-footer"><button style="margin-right: 20px;" data-role="none" data-ajax="false" type="button" '.($method=='create' && empty($exerciseRecord['goal_title']) ? 'data-onclick="checkTitleExist('."'".(!empty($assignId) ? $assignId : (!empty($logId) ? $logId : $fid)).'_'.$goalOrder."'".');"' : '').' class="btn btn-default confirm pointers" data-text="Clicking BACK or CANCEL will discard any changes. Clicking INSERT will apply any content changes. Continue with exiting?">close</button>';
					if($method!='preview'){
						if(!empty($type) && strtolower($type) == 'assigned' && !empty($foldid))
							$response .= 	'<button data-role="none" data-ajax="false" class="btn btn-default activedatacol" type="button" style="background-color:#fff" onclick="return editExercise('."'itemset".(!is_numeric($foldid) ? $foldid : '_'.$assignId.'_'.$foldid)."','".$foldid."'".');">insert</button>';
						elseif(!empty($type) && strtolower($type) == 'logged' && !empty($foldid))
			...
			$response 		.= '<div class="row">
								...
								<div class="row">
										...
										<div class="mobpadding">
											<div class="mobpadding">
												...
												<div id="createwkoutpopup" class="menuactive">
													<button data-ajax="false" type="button" data-role="none" class="btn btn-default" id="createwkoutbtn" onclick="return createNewExerciseSet();">
												...
												<div class="hide allowhide">
													<button data-ajax="false" data-role="none" class="btn btn-default" type="button" onclick="return createCopyExerciseSet('."'','down'".');">
									...
									<div class="mobpadding">
										<div id="scrollablediv-len" class="scrollablepadd col-xs-12 '.(isset($exerciseRecord) && count($exerciseRecord)>4 ? 'scrollablediv' : '').'">
											...
											<ul class="sTreeBase bgC4 '.(!empty($exerciseRecord) && count($exerciseRecord)>0 ? '' : 'hide').'" id="sTree3">';
					...
					$response .= '<div class="modal-footer"><div class="col-xs-12 activedatacol" style="float:right"><div class="col-xs-12 activedatacol"><button data-role="none" data-ajax="false" type="button" class="btn btn-default confirm pointers" data-text="Clicking BACK or CANCEL will discard any changes. Clicking MORE will display record options such as SAVE. Continue with exiting?">cancel</button><button data-role="none" type="submit" data-ajax="false" class="btn btn-default activedatacol" onclick="return confirmPopup();" name="f_method" style="background-color:#fff">more</button></div></div></div></div></form></div></div>';
					$response .= "<script type='text/javascript'> $('.time-picker').datetimepicker({ format:'hh:mm:ss' });$('.time-picker1').datetimepicker({ format:'mm:ss' });</script>";
			...
			$response = '<div class="modal-dialog modal-md exerciseAct">
							<div class="modal-content">
								<div class="modal-header">';
							$response .= '<div class="row">
											<div class="mobpadding">
						...
						$response .=   '<div class="opt-row-detail">
							...
							$response	.='">
									...
									$response .=' >
										...
										<div class="opt-row-detail">
											<a data-role="none" data-ajax="false" href="javascript:void(0);"  data-dismiss="modal" onclick="changeTosaveIcon();$('."'#createwkoutbtn'".').click();" class="btn btn-default" style="width:100%">
												<div class="col-xs-12 pointer">
													<div class="col-xs-2"><i class="fa fa-plus-square-o iconsize"></i></div>
													<div class="col-xs-10">Add a New Set</div>
												</div>
											</a>
										</div>
									</div>
					...
					<div id="myModalDuplicate" class="modal fade bs-example-modal-sm in" role="dialog" data-keyboard="false" data-backdrop="static">
						...
						$response .= '<div class="row">
										<div class="mobpadding">
											<div class="border">
												<div class="col-xs-2">
													<a data-role="none" data-ajax="false" href="javascript:void(0);" onclick="$('."'".'#exercisesetaction'."'".').show();$('."'".'#myModalDuplicate'."'".').hide();" class="triangle">
														<i class="fa fa-chevron-left iconsize"></i>
													</a>
												</div>
						...
						<div id="myeditWorkoutModal" class="modal fade bs-example-modal-sm" role="dialog" data-keyboard="false" data-backdrop="static"></div>';
		}elseif(!empty($action) && trim($action) == 'assignOptions'){
			$current = strtotime(date("Y-m-d"));
			$datediff = strtotime($date) - $current;
			$difference = floor($datediff/(60*60*24));
			$response = '<div id="changeassign" class="modal-dialog modal-md">
			...
			$response = '<div class="modal-dialog assignOptions">
							<div class="modal-content">
			...
			$response = '<div class="modal-dialog modal-md confirmWorkoutAct">
							<div class="modal-content">
						...
						$response .= '<div class="row">
								...
								$response .= '<div class="opt-row-detail">
											<button data-role="none" data-ajax="false" class="btn btn-default" type="button" '.(!empty($fid) ? (!empty($type) && $type == 'logged' ? 'onclick="saveLogWorkouts('."'0'".');"' : 'onclick="$('."'".($method=='confirmpopup' ? '#createNewworkout' : '#form-workoutrec')."'".').submit();"') : (!empty($type) && $type == 'logged' ? 'onclick="saveLogWorkouts('."'0'".');"' : (!empty($type) && $type == 'assigned' ? 'onclick="$('."'".($method=='confirmpopup' ? '#createNewworkout' : '#form-workoutrec')."'".').submit();"' : 'onclick="confirmwkout('."'".$modelType."','0'".');"'))).' style="width:100%">
												<div class="col-xs-12 pointer">
													<div class="col-xs-2"><i class="fa fa-pencil-square-o iconsize"></i></div>
			...
			$response = '<div class="modal-dialog modal-md confirmWorkoutLogAct">
							<div class="modal-content">
									...
									<div class="opt-row-detail">
										<a data-role="none" data-ajax="false" href="'.URL::base(TRUE).'exercise/workoutrecord/'.$fid.'/?act=add" class="btn btn-default" style="width:100%">
											<div class="col-xs-12 pointer">
												...
												<div class="col-xs-9">Add a New Set</div>
											</div>
										</a>
									</div>
								</div>
			...
			$response = '<div class="modal-dialog modal-md exerciseRecordAct">
							<div class="modal-content">
			...
			$response = '<div class="modal-dialog bs-example-modal-sm exerciseRecordAct">
								<div class="modal-content">
			...
			if($type =='myexercise' || $type =="sampleexercise"){
				$href = URL::base(TRUE).'exercise/exerciserecord/'.($type =='myexercise' ? 'startmyxr' : 'startsamplexr').'/'.$fid;
			}else{
			...
			$response = '<div class="modal-dialog modal-md">
									...
									<div class="row">
										<div class="mobpadding">
											<div class="border">
												...
												<div class="col-xs-8 optionpoptitle">Options for this '.($type !='myexercise' && $type !='sampleexercise' ? ($type=='wrkout' ? '' :($type=='sample' ? 'Sample' : ($type =='shared' ? 'Shared' : ($type =='assigned' ? 'Assigned' : 'Logged')))) : '').($type !='myexercise' && $type !='sampleexercise' ? ' Workout Plan' : ' Exercise Record').'</div>
												<div class="col-xs-2"></div>
											</div>
										</div>
									</div>';
					...
					}else if($type =='myexercise' || $type =='sampleexercise'){
						$response .='<div class="row opt-row-detail">
										<div class="col-xs-12">
											<a data-role="none" data-ajax="false" href="'.($requestFrom == 'dashboard' ? $href.'"' : 'javascript:void(0);" onclick="createNewExercise(true);"').' style="width:100%"  class="btn btn-default" data-request="'.$requestFrom.'">
			...
			$response = 	'<div id="workoutaction" class="modal-dialog modal-md">
								<div class="modal-content">
								<form data-ajax="false" action="" method="post" >
									<div class="modal-header">';
						$response .= '<input type="hidden" value="'.$fid.'" name="workout_id" />';
						...
						<div id="myModalDuplicate" class="modal fade bs-example-modal-sm" role="dialog" data-keyboard="false" data-backdrop="static">
								<div class="modal-dialog modal-md">
									<div class="modal-content">
			...
			$response = '<div class="modal-dialog modal-md otherWorkoutAction">
								<div class="modal-content">
			...
			if((($difference > 0 || $difference == 0 || $difference < 0) && (($type!='loggedwkout' && $type!='logged')  || $type=='')) || (($difference == 0 || $difference < 0) && $type!='' && ($type=='loggedwkout' || $type=='logged'))){
				...
				$response = '<div class="modal-dialog addAssignWorkouts">
									<div class="modal-content">
								...
								$response .= '<div class="row">
									...
									if(empty($type) || ($type!='logged' && $type != 'loggedwkout' && $type != 'workout')){
										if(isset($workoutRecord) &&  (!empty($fid) || !empty($assignId)) && $type!='dulicateAssignWkoutLog')
											$response .= '<button value="assignAdd" data-role="none" data-ajax="false" class="btn" type="submit" name="f_method" style="background-color:#fff"><i class="fa fa-check-square-o" style="font-size:30px;"></i></button>';
										...
										elseif($type == 'wkoutAssigncal' || $type =="duplicate" )
											$response .= '<button value="assignAdd" data-role="none" data-ajax="false" class="btn" type="submit" name="f_method" style="background-color:#fff"><i class="fa fa-check-square-o" style="font-size:30px;"></i></button>';
										elseif($type != '' )
											$response .= '<button value="assignAdd" data-role="none" data-ajax="false" class="btn" type="button" name="f_method" style="background-color:#fff" onclick="addDatetoAssign('."'".$date."','".$modelType."'".');"><i class="fa fa-check-square-o" style="font-size:30px;"></i></button>';
							...
							$response .=    '<div class="opt-row-detail">
									...
									if(isset($myworkoutDetails) && !empty($myworkoutDetails)){
										$response .='<ul class="'.(count($myworkoutDetails)>6 ? 'scrollwkout' : '').'">';
										...
										$response .= '<li><div class="opt-row-detail border-xr full">
														<div class="col-xs-12 pointer">
															<div class="col-xs-10" '.($type == 'logged' || $type == 'loggedwkout' ? 'onclick="addWorkoutlogs('."'".$date."','".(isset($values['parent_folder_id']) ? $values['parent_folder_id'] : '0')."','".$wkoutId."','".$method.($type=="logged" || $type == 'loggedwkout' || $type=="workout"  ? '-'.$type : '')."'".')"' : ($type == 'assigned' || $type==''  ? 'onclick="createNewworkoutAssign('."'".$date."','".(isset($values['parent_folder_id']) ? $values['parent_folder_id'] : '0')."','".$wkoutId."','".$method.($type=="logged" || $type == 'loggedwkout'|| $type=="workout" ? '-'.$type : '')."'".')"' : ($type == 'workout'  ? 'onclick="previewworkout('."'".$date."','".(isset($values['parent_folder_id']) ? $values['parent_folder_id'] : '0')."','".$wkoutId."','".$method."'".')"' : ''))).'>
																<div class="colorchoosen col-xs-2" style="float:left;margin-top:5px;">
																	<i class="glyphicon '.$values['color_title'].'" style="float:left"></i>
																</div>
																<div class="col-xs-9 wrapword text-left"><div>'.$values['wkout_title'].'</div><div class="wkoutfocus">'.ucfirst($values['wkout_focus']).'</div></div>
															</div>
															<div class="col-xs-2">'.'<i onclick="insertToWkout('."'".$wkoutId."','".$method."','".$type."','".$date."','".addSlashes(ucfirst($values['wkout_title']))."'".');" class="fa fa-sign-in iconsize"></i>'.'</div>
														</div>
													</div>
													</li>';
											}else{
														...
														<div class="col-xs-12 pointer">
															<a data-role="none" data-ajax="false" href="javascript:void(0)" onclick="addAssignWorkoutsByDate('."'".$date."','".$values['folder_id']."',"."'0','".$method.($type=="logged" || $type == 'loggedwkout'|| $type=="workout" ? '-'.$type : '')."'".')" style="width:100%">
																<div class="col-xs-10" style="padding: 0;">
																	<div class="colorchoosen col-xs-3" style="float:left"><i class="fa fa-folder-o iconsize" style="float:left"></i></div>
																	<div class="col-xs-9 text-left">'.$values['folder_title'].'<div class="navimgdet3">'.(($values['totalRecords'] > 0 )? (($values['totalRecords'] > 1 ) ? 'Records : <span>'.$values['totalRecords'].'</span>' : 'Record : <span>'.$values['totalRecords'].'</span>') : 'Record : <span>0</span>').'</div></div>
																</div>
																<div class="col-xs-2"></div>
															</a>
														</div>
					...
					$response .=    '</div>
						...
						</div><script type="text/javascript">$(document).ready(function(){ if($(".min-date"))$(".min-date").mobipick();});</script>';
			}else{
				$response = '<div class="modal-dialog addLogWorkouts">
									<div class="modal-content">
			...
			$response = '<div class="modal-dialog addAssignExercise">
							<div class="modal-content">
					...
					$response .=    '<div class="opt-row-detail">
										<a data-role="none" data-ajax="false" class="activedatacol" href="javascript:void(0)" onclick="createNewExercise();" style="width:100%" data-request="'.$requestFrom.'">
											<div class="col-xs-12 pointer">
												<div class="col-xs-2"><i class="fa fa-plus iconsize"></i></div>
									...
									<hr>
									<div class="opt-row-detail">
										<a data-role="none" data-ajax="false" href="javascript:void(0)" '.(count($xrRecDetailsmy) > 0 ? 'class="activedatacol" onclick="createExerciseFromXrLibrary('."'myexercise'".')"' : 'class="inactivedatacol" onclick="return false;"').' style="width:100%" >
											<div class="col-xs-12 pointer">
									...
									<hr>
									<div class="opt-row-detail">
										<a data-role="none" data-ajax="false" href="javascript:void(0)" '.(count($xrRecDetailssample) > 0 ? 'class="activedatacol" onclick="createExerciseFromXrLibrary('."'sampleexercise'".')"' : 'class="inactivedatacol" onclick="return false;"').' style="width:100%" >
											<div class="col-xs-12 pointer">
									...
									<div class="opt-row-detail hide">
										<a data-role="none" data-ajax="false" href="javascript:void(0)" '.(count($xrRecDetailsshared) > 0 ? 'class="activedatacol" onclick="createExerciseFromXrLibrary('."'sharedexercise'".')"' : 'class="inactivedatacol" onclick="return false;"').' style="width:100%" >
											<div class="col-xs-12 pointer">
			...
			$response = '';					
					$response .='<div id="workoutLogConfirm" class="modal-dialog addAssignWorkouts">
									<div class="modal-content">';
								...
								$response .='<div class="modal-header">
											<div class="row">
												<div class="mobpadding">
													<div class="border">
														<div class="col-xs-2">
															<a data-role="none" data-text="Clicking BACK or CANCEL will discard any changes. Clicking INSERT will apply any content changes. Continue with exiting?" data-ajax="false" href="javascript:void(0)" data-onclick="closeModelwindow('."'".(isset($modelType) ? $modelType : '')."'".');" class="triangle confirm pointers"><i class="fa fa-chevron-left iconsize"></i>
															</a>
														</div>
							...
							$response .= '</div>';
										if(!empty($type) && $type == 'loggedwkout'){
											$response .= '<div class="modal-footer"><button data-ajax="false" data-role="none" type="button" class="btn btn-default confirm pointers" data-text="Clicking BACK or CANCEL will discard any changes. Clicking INSERT will apply any content changes. Continue with exiting?" data-onclick="skipWorkoutNotesToLog('."'".$goalOrder."','".$modelType."'".');">Skip</button><button id="update_log" data-ajax="false" data-role="none" name="f_method" class="btn btn-default" value="update_log_conf" type="button" onclick="confirmLogDetails('."'".$goalOrder."'".');" ><i class="fa fa-pencil-square-o iconsize"></i></button></div>';
										}elseif(!empty($type) && ($type == 'loggedexercise' || $type == 'xrlog')){
											$response .= '<div class="modal-footer"><button data-ajax="false" data-role="none" type="button" class="btn btn-default confirm pointers" data-text="Clicking BACK or CANCEL will discard any changes. Clicking INSERT will apply any content changes. Continue with exiting?" data-onclick="skipExerciseNotesToLog('."'".$foldid."','".$goalOrder."','".$modelType."'".');">Skip</button><button id="update_log" data-ajax="false" data-role="none" name="f_method" class="btn btn-default" value="update_log" type="button" onclick="confirmExerciseDetails('."'".$foldid."','".$goalOrder."'".');" ><i class="fa fa-pencil-square-o iconsize"></i></button></div>';
										}elseif(!empty($type) && $type == 'loggedexerciseconf'){
											$response .= '<div class="modal-footer"><button data-ajax="false" data-role="none" type="button" class="btn btn-default confirm pointers" data-text="Clicking BACK or CANCEL will discard any changes. Clicking INSERT will apply any content changes. Continue with exiting?" data-onclick="skipExerciseNotes('."'".$foldid."','2','".$modelType."'".');">Skip</button><button id="update_log" data-ajax="false" data-role="none" name="f_method" class="btn btn-default" value="update_log" type="button" onclick="confirmExerciseDetails('."'".$foldid."','".$goalOrder."'".');" ><i class="fa fa-pencil-square-o iconsize"></i></button></div>';
										}else if(!empty($type) && $type == 'wkoutLogFromPrev'){
											$response .= '<div class="modal-footer"><button id="add_new_log_end" data-ajax="false" data-role="none" name="f_method" class="btn btn-default confirm" data-text="Clicking BACK or CANCEL will discard any changes. Clicking INSERT will apply any content changes. Continue with exiting?" value="add_new_log_end" type="submit">Skip</button><button id="add_new_log_start" data-ajax="false" data-role="none" name="f_method" class="btn btn-default" value="add_new_log_start" type="submit" ><i class="fa fa-pencil-square-o iconsize"></i></button></div>';
										}else{
											$response .= '<div class="modal-footer"><button id="add_new_log_end" data-ajax="false" data-role="none" name="f_method" class="btn btn-default confirm pointers" data-text="Clicking BACK or CANCEL will discard any changes. Clicking INSERT will apply any content changes. Continue with exiting?" data-onclick="confirmLogDetails('."'".($goalOrder >=0 ? $goalOrder : '2')."'".');" value="add_new_log_end" type="button">Skip</button><button id="add_new_log_start" data-ajax="false" data-role="none" name="f_method" onclick="confirmLogDetails('."'".($goalOrder >=0 ? $goalOrder : '1')."'".');" class="btn btn-default" value="add_new_log_start" type="button" ><i class="fa fa-pencil-square-o iconsize"></i></button></div>';
										}
							$response .= '</form></div>
			...
			$response = '<div class="modal-dialog bs-example-modal-sm">
							<div class="modal-content">
							...
							if(!empty($title)){
								...
								$response .= '<input type="text" class="form-control xrtag-input-tag" name="xrtag-input" value="" placeholder="Tags" data-role="tagsinput"/> ';
							}
							$response .='</div>';
							$current_site_id = (Session::instance()->get('current_site_id') ? Session::instance()->get('current_site_id') : '0');
							...
							$response .='<div class="form-group" '.(Helper_Common::is_admin() || Helper_Common::is_manager() ? 'style="display:block"' : 'style="display:none"').'>
											...
											<input type="text" class="input-lg form-control fullwidth" id="wkout_site_names" name="seletedSite[]" style="width: 100%;"/>
										</div>';							
							$response .='<div class="form-group">
											<label class="control-label" for="recipient-name">Recipient(s):</label>
											<input type="text" class="input-lg form-control fullwidth" id="wkout_user_names" name="seletedUser[]" style="width: 100%;"/>
										</div>';
							$response .='<div class="form-group">
			...
			if($method == 'relatedRecords'){
				...
				if($addOptions == 'add' && !empty($fid) && is_numeric($fid))
			...
			$response = '<div class="modal-dialog bs-example-modal-sm">
							...
							$response .= '<div class="row">
										...
										if($method =='xrrate'){
											...
											elseif($rateFlag)
												...
												$response .= '<button style="background-color:#fff" name="f_method" class="btn" data-ajax="false" type="submit" onclick="closeModelwindow('."'".$modelType."'".');" data-role="none"><i style="font-size:30px;" class="fa fa-times"></i></button>';
										}
										$response .='</div>
												</div>';
							$response .=  '</div>
										...
										if(!empty($addOptions) && $addOptions == 'add'){
											...
											<div class="opt-row-detail">
												<button class="btn btn-default" onclick="triggerXrTagModal();" type="button" style="width:100%" data-ajax="false" data-role="none">
													<div class="col-xs-12 pointer">
														<div class="col-xs-2"><i class="fa fa-tag iconsize"></i></div>
														<div class="col-xs-10">'.__('Tag this Record').'</div>
													</div>
												</button>
											</div>
											<div class="opt-row-detail">
												<button class="btn btn-default" onclick="return getRateForXrModalFromUser('."'".$fid."'".');" type="button" style="width:100%" data-ajax="false" data-role="none">
													<div class="col-xs-12 pointer">
														<div class="col-xs-2"><i class="fa fa-star-o iconsize"></i></div>
														<div class="col-xs-10">'.__('Rate this Exercise').'</div>
													</div>
												</a>
											</div>';
										}
					...
					$response .= '</div>'.($method !='tagRecord' ? '<div class="modal-footer"><button data-role="none" data-ajax="false" type="button" class="btn btn-default" data-dismiss="modal">close</button></div>' : '').'
						...
						</div><div id="myOptionsModalExerciseRecord_more" class="modal fade bs-example-modal-sm" role="dialog" data-keyboard="false" data-backdrop="static"></div>
					<div id="myOptionsModalExerciseRecord_option" class="modal fade bs-example-modal-sm" role="dialog" data-keyboard="false" data-backdrop="static"></div>';
			...
			$response = '<div class="modal-dialog confirmAssignDate">
									<div class="modal-content">
										...
										<div class="modal-header">
											<div class="row">
												<div class="mobpadding">
													<div class="border">
														<div class="col-xs-2">
															<a data-role="none" data-ajax="false" href="javascript:void(0)" onclick="openBackwindow('."'".$modelType."'".');" class="triangle first-col"><i class="fa fa-chevron-left"></i>
															</a>
															<a data-role="none" data-ajax="false" href="javascript:void(0)" onclick="openDateCalender('."'".$date."','1'".');" class="triangle second-col hide"><i class="fa fa-chevron-left iconsize"></i>
															</a>
														</div>
														<div class="col-xs-8 optionpoptitle">Specify the Date</div>
														<div class="col-xs-2 activedatacol second-col hide">
															<button style="background-color:#fff" name="f_method" onclick="return addAssignWorkoutsByDate('."$('#selected_date').val(),'0','".(!empty($assignId) ? $assignId : (!empty($logId) ? $logId : (!empty($fid) ? $fid : '0')))."','".(!empty($assignId) ? 'assigned' : (!empty($logId) ? 'wkoutlog' : (!empty($type) ? $type : '')))."'".');" class="btn" data-ajax="false" type="button" data-role="none"><i style="font-size:30px;" class="fa fa-check-square-o"></i></button>
														</div>
													</div>
												</div>
											</div>
										</div>';
										$response .= '<div class="modal-body opt-body">
														...
														<div class="opt-row-detail second-col hide">
															<div class="aligncenter">Selected Date:</div>
															<input type="text" id="selected_date" '.(!empty($type) && ($type=='logged-create' || $type=='dulicateWkoutLog') ? 'max="'.date("d M Y").'"' : 'min="'.date("d M Y").'"').' value="'.date("d M Y",strtotime($date)).'" class="form-control input-lg min-date"/>
														</div>
														<br>
													  </div>';
									$response .='<div class="modal-footer"><button data-ajax="false" data-role="none" type="button" class="btn btn-default" data-dismiss="modal">cancel</button></div>
									</form>
									</div>
								</div>
							<script type="text/javascript">$(document).ready(function(){$(".min-date").mobipick();});</script>';
		}elseif(!empty($action) && trim($action) == 'xrRecordactions'){
			$response = '<div class="modal-dialog xrRecordactions">
							<div class="modal-content">
			...
			$response = '<div class="modal-dialog editNotification">
							<div class="modal-content">
								<form action="" method="post" id="editNotification" data-ajax="false">
								<div class="modal-header">
									<div class="row">
										<div class="mobpadding">
											<div class="border">
								...
								<div class="modal-body">
									<div class="col-xs-12">
										<p>List Edit Mode provides the following options for managing the existing </p>
										<p>exercise set entries:</p>
										</br>
										<p><span class="fa fa-arrows"></span> Move (drag) to re-order an entry</p>
										<p><span class="fa fa-files-o"></span> Clone (copy) any selected entries</p>
										<p><span class="fa fa-times"></span> Delete any selected entries</p>
										</br>
										<p>To return to Sets Edit Mode, tap <span class="fa fa-refresh"></span> "sets/list" 
										button again. This will allow you to create or edit entries for individual 
										exericse sets.</p>
									</div>
								</div>
								<div class="modal-footer"><button data-ajax="false" data-role="none" type="button" class="btn btn-default" data-dismiss="modal">close</button></div>
			...
			$response = '
								<div class="modal-dialog modal-md">
									...
									$response .= '<div class="opt-row-detail">
											...
											<div class="opt-row-detail">
												<a onclick="createExercise();" class="btn btn-default" style="width:100%" data-ajax="false" data-role="none">
													<div class="col-xs-12 pointer">
			...
			if($method  == 'profileedit'){
				...
				$title = "Cancle My Account Confirmation";
			...
			$response = '<div class="modal-dialog">
				...
				if($method  == 'profileedit'){
						$response .= '<div class="workoutprofile">
									...
									<div class="row" data-role="none">
										<div class="col-xs-12">
											<input style="width:100%;background-color: #eee;opacity: 1;" id="dobch" date-role="none" type="text" '.($fromAdmin == '1' ? 'data-format="dd/MM/yyyy"' : '').'  required="true" class="usernamebutton min-date add-on" value="'.($fromAdmin == '1' ? $userdetails->user_dob : date("d M Y",strtotime($userdetails->user_dob.' 00:00:00'))).'" name="dobch"/>
						...
						$response .= HTML::script('assets/js/jquery.flot.js').HTML::script('assets/js/jquery.flot.resize.js').HTML::script('assets/js/jquery.flot.pie.min.js').'<div class="container-fluid">
										<div class="row">
											<div class="col-lg-5">';
											if($userimageid != ''){ 
				...
				if($method  == 'profilecancel'){
					...
					$response .='<div class="modal-footer">
								   <button data-role="none" data-ajax="false" type="button"  class="btn btn-default" onclick="showUserModel('.$userdetails->pk().',1)">close</button>
				...
				$response .= '</form></div>
						  </div><div id="userModalActions"  class="modal fade" role="dialog" data-keyboard="false" data-backdrop="static"></div>';
			//if($fromAdmin ==false)
				$response .= '<script type="text/javascript">$(document).ready(function(){ if($("#fusernamech"))$("#fusernamech").addClass("usernamebutton"); if($("#lusernamech"))$("#lusernamech").addClass("usernamebutton"); });</script>';
		}
