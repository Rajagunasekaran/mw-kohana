...
class Controller_Search extends Controller {
	...
	$response .=					'</div>
			...
			if(empty($modelType))
				$modelType = 'myModal';
			$response = $XRciselib.'<div class="modal-dialog '.($method == 'title' ? 'hide' : '').'" '.($method == 'title' ? 'id="title-div"' : '').'><div class="modal-content">
				...
				<button style="margin-right: 20px;" data-role="none" data-ajax="false" type="button" class="btn btn-default" onclick="closeModelwindow('."'".$modelType."'".');">close</button><button data-role="none" data-ajax="false" class="btn" onclick="insertExtraToParentHidden('."'".$modelType."'".');" name="f_method" style="background-color:#fff" type="button"><i class="fa fa-sign-in" style="font-size:30px;" ></i></button></div></form></div></div>';
					...
					$('.time-picker1').datetimepicker({ format:'HH:mm:ss' });$('.time-picker2').datetimepicker({ format:'mm:ss' }); ".(!empty($exerciseRecord['goal_unit_id']) ? " if($('#exerciselib')) { $('#exerciselib').bootstrapSwitch('state', true); $('#exerciselib-model').modal(); 
						...
						$('.checkboxdrag').on('switchChange.bootstrapSwitch', function (event, state) {
							if(state === true) { if($('#".$method."-div').length) $('#".$method."-div').addClass('hide'); $('#".$method."').val('on'); $('#".$method."_hidden').val('on'); }
							else { $('#".$method."').val('off');  $('#".$method."_hidden').val('off');  $('#".$method."_hidden').val('off'); if($('#".$method."-div').length) $('#".$method."-div').removeClass('hide');}
						});" 
						: 
						"
						$('.checkboxdrag').on('switchChange.bootstrapSwitch', function (event, state) {
							if(state === true) { if($('#".$method."-div').length) $('#".$method."-div').addClass('hide');$('#".$method."').val('on'); $('#".$method."_hidden').val('on'); }
							else { $('#".$method."').val('off');  $('#".$method."_hidden').val('off'); $('#".$method."-div').removeClass('hide');}
						});
						")." if($('#exerciselib')){
							...
							<div class="modal-content">
								...
								<div class="modal-body">
									<div class="col-xs-12">
										...
										'.($method =='workout' ? '<p>Workout Plans Entries:</p>' : '<p>Exercise Set Entries:</p>'). '
										</br>
										...
										'.($method !='workout' ? '</br><p>To return to Sets Edit Mode, tap <span class="fa fa-refresh"></span> "sets/list" button again. This will allow you to create or edit entries for individual exericse sets.</p>' : '' ).'</div>
								</div>
								<div class="modal-footer"><button data-ajax="false" data-role="none" type="button" class="btn btn-default" data-dismiss="modal">close</button></div>
								</form>
							</div>
