...
class Kohana_URL {
	...
	public static function base($protocol = NULL, $index = FALSE)
	{
		...
		$siteUrl =''; 
		if($protocol === TRUE && Request::current()->directory() != 'Admin' && Request::current()->directory() != 'Site' && Request::current()->controller() != 'Index'){
			$siteUrl = (Session::instance()->get('current_site_slug') ? Session::instance()->get('current_site_slug').'/' : '');
		}
		
		//echo $base_url."========================".$protocol."==========".Request::current()->directory()."===77777777777===========".$siteUrl;
		//die();
		$base_url .= $siteUrl;
		if ($protocol === TRUE)
		{
	...
	public static function site($uri = '', $protocol = NULL, $index = TRUE)
	{
		...
		$returnUrl = URL::base($protocol, $index).$path;
		$siteUrl = '';
		if($protocol === TRUE && Request::current()->directory() != 'Admin' && Request::current()->directory() != 'Site' && stripos($path,'assets/') !==false ){
			$siteUrl = (Session::instance()->get('current_site_slug') ? Session::instance()->get('current_site_slug').'/' : '');
		}
		$returnUrl = str_replace($siteUrl,'',$returnUrl);
		// Concat the URL
		return $returnUrl;
	}
	...
	{
		$site_title = (Session::instance()->get('current_site_name') ? Session::instance()->get('current_site_name').' - ' : '');
		$title = $site_title;
		if ($ascii_only === TRUE)
		{
