...
class Kohana_Email {
	 ...
	 * @return  object  Swift object
	 */
	public static function dynamicMailer($driver = array(),$options = array())
	{
		if ( ! Email::$_mailer)
		{
			// Load email configuration, make sure minimum defaults are set
			if(count($driver) > 0 && count($options) > 0){
				$config = array('driver'  => $driver,	'options' => $options);
			}else{
				$config = array('driver'  => 'native',	'options' => array());
			}
	...
	public static function sendBysmtp($mailler, $messageArray = array())
	{
		$message = Swift_Message::newInstance($messageArray['subject']);
		
		$message->setFrom(array($messageArray['from']=>$messageArray['fromname']));
      $message->setTo(array($messageArray['to']=>$messageArray['toname']));
		if(isset($messageArray['cc'])){
			$message->setCc(array($messageArray['cc']=>$messageArray['ccname']));
		}
		$message->addPart($messageArray['body'],$messageArray['type']);
		$message->setReplyTo(array($messageArray['from']=>$messageArray['fromname']));
		if($mailler)
