<div id="exerciselib-template">
	...
	function xrLibcreateNewExercise(opt) {
		...
		$.ajax({
			url: siteUrl_frontend + "ajax/getAjaxExerciseCreateHtml",
			type: 'post',
			data : {
				xrid : xrrecid,
	...
	$(document).ready(function(){
		if(fileList.length > 0){
			fileList.bind('scroll', function(e){
				$('html, body').animate({scrollTop: fileList.position().top}, 'slow');
	...
	function xrLibAutoShowMore(act) {
		if(act == 'onclick'){
			var x = 1,
			...
			success : function(content){
				...
				if(ajaxData.content!=''){
					...
					$("input#xr_user_names").select2({ placeholder: "Search Users", minimumInputLength: 2, multiple: true,
						ajax: { url: siteUrl_frontend + 'search/getajax/',
	...
	function shareExerciseRecord() {
		$.ajax({
			url: siteUrl_frontend + 'ajax/shareExerciseFromXrciseModal',
			method: 'post',
