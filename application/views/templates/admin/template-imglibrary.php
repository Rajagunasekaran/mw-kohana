<div class="imagelib-template">
								...
								<div class="row">
									...
									<div class="col-sm-12">
										<div class="dropdown selectdropdownTwo">
											<select class="mdl_fltrsort-select selectAction" id="mdl_fltrsort-select" data-ajax="false" data-role="none" style="width:50% !important;">
	...
	function initSimpleUpload() {
		...
		var mdl_imguploader = new ss.SimpleUpload({
			...
			url: siteUrl + 'image/uploadImg?action=upload',
			name: 'uploadfile',
	...
	/*for tag*/
	$(document).ready(function(){
		if(!$('form#xrRecInsertForm').length){
			var tagarry = [];
			$.ajax({
	...
	function popuptriggerImgPrevModal(elem){
		...
		if(imgurl!=undefined && imgurl!=''){
			$('#mdl_preview_libimg').html('<img alt="<?php echo __('Preview Image'); ?>" class="Preview_image" id="mdl_previewlibimg" src="' + siteUrl_frontend + imgurl + '"/>');
	...
	function popuptriggerAjaxImgLibrary(param){
		...
		$('.temp-banner').empty();
		$.ajax({
			url: siteUrl + 'ajax/getAjaxImgLibraryHtml' + (param == 'init' ? '?user_from=admin&cp='+user_allow_page : ''),
			type: 'post',
			async: false,
			encode: true,
			cache: false,
				...
				var rec = '<li class="imgRecord" id="'+f.img_id+'">';
					rec += '<div class="imgRecordDataFrame col-xs-12 col-sm-12">';
						rec += '<a href="javascript:void(0);" class="col-xs-10 col-sm-10 imgFrame-left" data-ajax="false" data-role="none">';
							rec += '<div class="col-xs-4 col-sm-4 mdl_thumb-img" '+attribute+' onclick="popuptriggerImgPrevModal(this);"'+(testedimg != '' ? ' style="background-image: url('+siteUrl_frontend + testedimg+');"' : '')+ '>'+dummyicom+'</div>';
	...
	$(document).on('click','#mdl_imginsert-btn, .insert-this-img',function(){
		...
		if($("#triggerid").length > 0 && $("#triggerid").val() == 4){
			if($("#triggerid").attr('data-imgtagid')!='' && $("#triggerid").attr('data-hiddenid')!=''){
				imgtagid = $("#triggerid").attr('data-imgtagid');
				hidnimgid = imgtagid;
				hiddenimgid = $("#triggerid").attr('data-hiddenid');
				$('#'+hiddenimgid).val($('#mdl_curr_imgid').val());
			}else{
				imgtagid = 'profile_im';
				hidnimgid = imgtagid;
			}
		...
		if(imglink!=undefined && imglink!=''){
			$('#'+imgtagid).attr('src', siteUrl_frontend + imglink);
		}else{
			$('#'+imgtagid).attr('src', siteUrl_frontend + 'assets/images/icons/picture-icon.png');
		}
