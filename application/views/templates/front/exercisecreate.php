<div class="exercisecreate-template">
	<div id="exercisecreate-modal" class="modal fade" role="dialog" tabindex="-1">
		<div class="vertical-alignment-helper">
			<div class="modal-dialog">
				<div class="modal-content" id="xrRec-container"></div>
			</div>
		</div>
	</div>
	<!-- xr saveing modal -->
	<div id="xrcisesaveopt-modal" class="modal fade" role="dialog" tabindex="-1">
		<div class="vertical-alignment-helper">
				...
				<div class="modal-content">
					<div class="modal-header">
						<div class="mobpadding">
							<div class="border">
								<div class="col-xs-2">
									<a href="javascript:void(0);" title="<?php echo __('Back'); ?>" onclick="$('#xrcisesaveopt-modal').modal('hidecustom');" class="triangle" data-ajax="false" data-role="none">
					...
					<div class="modal-body opt-body">
						<div class="opt-row-detail hide">
							<button class="btn btn-default" id="btn_trgrinserttoset" onclick="" type="button" style="width:100%" data-ajax="false" data-role="none">
								<div class="col-xs-12 pointer">
									<div class="col-xs-3"><i class="fa fa-sign-in iconsize"></i></div>
									<div class="col-xs-9 change-text"><?php echo __('Insert into Exercise Set'); ?></div>
								</div>
							</button>
						</div>
						<div class="opt-row-detail">
							<button class="btn btn-default" id="btn_trgrsave" onclick="triggerXrFormSave();" type="button" style="width:100%" data-ajax="false" data-role="none">
								<div class="col-xs-12 pointer">
									...
									<div class="col-xs-9 change-text"><?php echo __('Save & Insert into Exercise Set'); ?></div>
								</div>
							</button>
						</div>
						<div class="opt-row-detail">
						...
						<div class="opt-row-detail">
							<a href="javascript:void(0);" class="btn btn-default confirm" id="btn_revertdata" data-onclick="triggerXrFormResettoSaved();" data-allow="<?php echo (Helper_Common::getAllowAllAccessByUser((Session::instance()->get('user_allow_page') ? Session::instance()->get('user_allow_page') : '1'), 'is_confirm_xr_create_hidden') ? 'false' : 'true'); ?>" data-notename="hide_confirm_xr_create" data-text="Clicking BACK or CANCEL will discard any changes. Clicking MORE will display record options such as SAVE. Continue with exiting?" style="width:100%" data-ajax="false" data-role="none">
								<div class="col-xs-12 pointer">
						...
						<div class="opt-row-detail hide">
							<button class="btn btn-default" id="btn_addrecord" onclick="" type="button" style="width:100%" data-ajax="false" data-role="none">
								<div class="col-xs-12 pointer">
	...
	<!-- preview xr feature image modal -->
	<div id="previewimg-modal" class="modal fade" role="dialog" tabindex="-1">
		<div class="vertical-alignment-helper">
			<div class="modal-dialog">
				<div class="modal-content aligncenter">
					<div class="modal-header">
						<div class="row">
							<div class="popup-title">
								<div class="col-xs-2">
									<a href="javascript:void(0);" title="<?php echo __('Back'); ?>" class="triangle" onclick="$('#previewimg-modal').modal('hidecustom');" data-ajax="false" data-role="none">
										<i class="fa fa-caret-left iconsize"></i>
									</a>
								</div>
								<div class="col-xs-8"><?php echo __('Exercise Record - Preview Image'); ?></div>
								<div class="col-xs-2"> </div>
							</div>
						</div>
					</div>
					...
					<div class="modal-footer">
						<button type="button" class="btn btn-default" onclick="$('#previewimg-modal').modal('hidecustom');" data-ajax="false" data-role="none"><?php echo __('Close'); ?></button>
					</div>
				</div>
			</div>
		</div>
	...
	<!-- img option modal -->
	<div id="imageoption-modal" class="modal fade" role="dialog" tabindex="-1">
		<div class="vertical-alignment-helper">
			<div class="modal-dialog modal-md">
				<div class="modal-content">
					<div class="modal-header">
						<div class="mobpadding">
							<div class="border">
								<div class="col-xs-2">
									<a href="javascript:void(0);" title="<?php echo __('Back'); ?>" onclick="$('#imageoption-modal').modal('hidecustom');" class="triangle" data-ajax="false" data-role="none">
										<i class="fa fa-chevron-left"></i>
									</a>
								</div>
					...
					<div class="modal-body opt-body">
						<div class="opt-row-detail">
							<a href="javascript:void(0);" id="btn_imgedit" class="btn btn-default edit-img cboxElement" style="width:100%" data-ajax="false" data-role="none">
								<div class="col-xs-12 pointer">
						...
						<div class="opt-row-detail">
							<a href="javascript:void(0);" id="btn_imgpreview" class="btn btn-default img_mgr featimgprev" onclick="triggerImgPreviewModal('intro_feature');" style="width:100%" data-ajax="false" data-role="none">
								<div class="col-xs-12 pointer">
						...
						<div class="opt-row-detail">
							<a href="<?php echo URL::base().'assets/images/icons/picture-icon.png'; ?>" id="btn_imgclear" data-clearid="" class="btn btn-default img_mgr img_clear" style="width:100%" data-ajax="false" data-role="none">
								<div class="col-xs-12 pointer">
									<div class="col-xs-3"><i class="fa fa-refresh iconsize"></i></div>
					...
					<div class="modal-footer">
						<button type="button" class="btn btn-default" onclick="$('#imageoption-modal').modal('hidecustom');" data-ajax="false" data-role="none"><?php echo __('Close'); ?></button>
					</div>
				</div>
			</div>
		</div>
	</div>
	<div id="preview-xr-modal" class="modal fade" role="dialog" tabindex="-1"></div>
	<div id="preview-xr-modal-options" class="modal fade" role="dialog" tabindex="-1"></div>
	<script type="text/javascript">
		$('#exercisecreate-modal').on('shown.bs.modal', function(){
			setTimeout(function(){
				$('#xru_title').focus();
			}, 400);
		});
		...
		function clearImg(clicked) {
			...
			startPoint.find('.uploaded_image_thmb').attr('src', noImgUrl).end().find('.img_selected').val('').end().find('.img_preview').attr('src', '<?php echo URL::base(); ?>'  + noImgUrl).end();
		...
		var icon = '<?php echo URL::base()."assets/images/icons/picture-icon.png"; ?>';
		...
		$(document).on('click change', '.checkboxcolor label input[type="checkbox"]', function() {
		...
		function triggerCloseXrciseCreateModal() {
			$('#exercisecreate-modal').modal('hidecustom');
			$('#btn_revert').trigger('click');
			if(($('#exerciselib-model #ref-flag').val()=='' && $('#exerciselib-model #act-flag').val()=='') || ($('#exerciselib-model #ref-flag').val()==undefined && $('#exerciselib-model #act-flag').val()==undefined)){
				if($('#xrRecInsertForm #actionflag').val()=='exercise'){
					createExercise();
				}else if($('#xrRecInsertForm #actionflag').val()=='workouts'){
					xrLibCreateExercise();
				}
			}
		}
		...
		function triggerXrFormResettoSaved() {
			$('#xrcisesaveopt-modal').modal('hidecustom');
			var actionflag = $('#xrRecInsertForm #actionflag').val();
			if(actionflag=='exercise'){
				createNewExercise('reset');
			}else{
				xrLibcreateNewExercise('reset');
			}
		}
		/*validate the feat-img field on submit*/
		$('#xrRecInsertForm').submit(function(ev) {
			// ev.preventDefault();
			$('#xrRecInsertForm').formValidation('revalidateField', 'xru_featImage');
		});
		/*create xr-rec from insert option modal from xr-lib*/
		/*insert to xr-set for workouts page*/
		function createExerciseFromCopy() {
			$.ajax({
				type: 'POST',
				url: '<?php echo URL::base()."ajax/CreateExerciseRecord"; ?>',
				data: {
					'addtype': $('#addtype').val(),
					'addid': $('#addid').val(),
					'action': 'addExercise'
				...
				success: function(data) {
					...
					if (response[0].flag) {
						insertRecordFromAdded(response[0].xrid, response[0].title, response[0].img_url);
		...
		/*xr-rec preview from create from - xr-lib modal */
		function getExercisepreviewOption(exerciseId, type, method){
			$('#preview-xr-modal').html();
			$.ajax({
				url : '<?php echo URL::base()."search/getmodelTemplate"; ?>',
				data : {
					...
					foldid : '0',
					type: type,
					actionFrom: $('#exerciselib-model #act-flag').val()
				},
				success : function(content){
					$('#preview-xr-modal').html(content);
					$('#preview-xr-modal').modal();
				}
			});
		}
		/*form validation*/
		...
		function WizardFormValidation() {
			$('#xrRecInsertForm').formValidation({
				...
				var submitval = fv.getSubmitButton().val();
				var request = $('#xrRecInsertForm #requestflag').val();
				var action = $('#xrRecInsertForm #actionflag').val();
				if(request == 'workouts'){
					$.ajax({
						type: 'POST',
						url: '<?php echo URL::base()."ajax/CreateExerciseRecord"; ?>',
						data: $form.serialize() + '&action=create&type=' + submitval,
						success: function(data) {
							$('#xrRecInsertForm > .bannermsg').remove();
							var response = [JSON.parse(data)];
							if (response[0].flag) {
								if(submitval == 'save'){
									setTimeout(function(){
									...
									$('#btn_revert').trigger('click');
								}else if(submitval == 'save-edit'){
									$('#xrRecInsertForm #xrid').val(response[0].xrid);
									$('#btn_trgrinserttoset').attr('onclick', "insertRecordFromAdded('"+response[0].xrid+"', '"+response[0].title+"', '"+response[0].img_url+"');");
									setTimeout(function(){
										if(action=='exercise'){
											createNewExercise('edit');
										}else{
											xrLibcreateNewExercise('edit');
										}
									}, 450);
								}
								if(response[0].act != undefined && response[0].act == 'updated'){
									setTimeout(function() {
										$('#xrRecInsertForm').prepend('<div class="row bannermsg"><div class="col-xs-12 col-sm-12 banner success">'+__('Exercise record modified successfully ')+' !!!</div></div>');
									}, 500);
								}else if(response[0].act != undefined && response[0].act == 'created'){
									setTimeout(function() {
										$('#xrRecInsertForm').prepend('<div class="row bannermsg"><div class="col-xs-12 col-sm-12 banner success">'+__('Exercise record created successfully ')+' !!!</div></div>');
									}, 500);
								}
							}else{
								$('#xrRecInsertForm').prepend('<div class="row bannermsg"><div class="col-xs-12 col-sm-12 banner errors error-color">'+__('Error occured, can\'t create this record.<br>Please try again later')+' !!!</div></div>');
							}
							setTimeout(function() {
								$('#xrRecInsertForm > .bannermsg').fadeOut(10000);
							}, 550);
							$('#xrRecInsertForm').data('formValidation').resetForm();
						}
					});
		...
		$('#xrcisesaveopt-modal').on('show.bs.modal', function(){
			if($('#xrRecInsertForm #xrid').val()!='' && $('#xrRecInsertForm #requestflag').val()=='workouts'){
				$('#btn_trgrinserttoset').parent('div').removeClass('hide');
			}else{
				$('#btn_trgrinserttoset').parent('div').addClass('hide');
			}
			var actionflag = $('#xrRecInsertForm #actionflag').val();
			if(actionflag=='exercise'){
				$('#xrcisesaveopt-modal #btn_addrecord').attr('onclick', 'createNewExercise();');
			}else{
				$('#xrcisesaveopt-modal #btn_addrecord').attr('onclick', 'xrLibcreateNewExercise();');
			}
		});
		...
		$(document).on('click keyup', '.tags-block .bootstrap-tagsinput', function() {
			if ($('#exercisecreate-modal #xrRecInsertForm').is(':visible')) {
				if ($('#exercisecreate-modal #xrRecInsertForm .tt-menu.tt-open').is(':visible')) {
					$('#exercisecreate-modal .modal-body').animate({
						scrollTop: $('#exercisecreate-modal .modal-body')[0].scrollHeight
					}, 1000);
				}
			}
		});
