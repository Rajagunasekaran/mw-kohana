<div>
  <div class="container">
	...
	<input type="hidden" value="<?php echo (isset($workoutRecord['parent_folder_id']) && (!isset($startJournal) || empty($startJournal)) ? $workoutRecord['parent_folder_id'] :'0');?>" name="parent_folder_id"/>
	...
	<div class="row">
		<div class="border">
			<div class="col-xs-3 aligncenter">
				<a data-ajax='false' data-onclick="$('#save_edit').val(5);$('#form-workoutrec').submit();" data-text="<?php echo __('Clicking BACK or CANCEL will discard any changes. Clicking MORE will display record options such as SAVE. Continue with exiting?');?>" href="javascript:void(0);" class="triangle confirm" data-allow="<?php echo (Helper_Common::getAllowAllAccessByUser($session->get('user_allow_page'),'is_confirm_wkout_hidden') ? 'false' : 'true');?>" data-notename="hide_confirm_wkout">
					...
					<div class="col-xs-9 activedatacol alignleft">
						<div class="pointers colormodelpopup">
							...
							<div data-toggle="collapse" data-target=".navbar-collapse-colors">
								<i id="wrkoutcolortext" class="glyphicon wrkoutcolor <?php echo $workoutRecord['color_title'];?>"style="float:left;width:20px;height:20px;top:4px;margin-left:8px;"></i>
								<input type="hidden" name="wrkoutcolor" id="wrkoutcolor" value="<?php echo $workoutRecord['color_id'];?>" >
							</div>
							<div><span data-toggle="collapse" data-target=".navbar-collapse-colors" ><i class="fa iconsize fa-caret-down" style="margin-left:8px;"></i></span></div>
...
<?php
	...
	foreach($exerciseRecord['uniqueset'] as $keys => $values){
		$order++;
		$exactOrder = $order.'_'.str_replace($overallorder.'_','',$keys);
?>
	<li data-orderval="<?php echo $order;?>" data-role="none" class="bgC4" data-inner-cnt="<?php echo (isset($exerciseRecord['setdetails'][$keys]) ? count($exerciseRecord['setdetails'][$keys]) : 0);?>" data-id='<?php echo str_replace($overallorder.'_','',$keys);?>' data-module="<?php echo (isset($exerciseRecord['setdetails'][$keys]) && count($exerciseRecord['setdetails'][$keys])>1 ? 'item_sets' : 'item_set');?>" id="itemSet_<?php echo $wkout_id.'_'.$exactOrder;?>"><!--wrkoutid.'_'.order.'_'.goal_unit_id-->
		<div id="itemset_<?php echo $exactOrder;?>" class="row">
			<input type="hidden" id="<?php echo 'goal_remove_combine_'.$exactOrder;?>" name="goal_remove_combine[<?php echo $exactOrder;?>]" value="0"/>
			...
			<div class="mobpadding">
				<div class="border full">
					...
					<div class="col-xs-8 navdescrip row-no-padding">
						<div class="col-xs-4 navimage activelink datacol row-no-padding " <?php if($values['goal_unit_id']){?> onclick="getTemplateOfExerciseRecordAction(<?php echo $values['goal_unit_id'];?>,this);" <?php } ?>>
						...
						<div class="col-xs-8 pointers activelink datacol row-no-padding">
							<div class="navimagedetails">
								<div class="navimgdet1" onclick="editWorkoutRecord('<?php echo $keys;?>','preview');"><b><?php echo (($values['goal_alt'] > 0) ? '<span>Alt</span> ' : '');?><?php echo ucfirst($values['goal_title']);?></b></div>
								...
								<div data-id="<?php echo $order.'_'.$value['goal_id'];?>" class="navimgdet2" id="set_id_<?php echo $order.'_'.$value['goal_id'];?>">
									...
									<?php 
										...
										if($value['goal_rate']>0 && $value['goal_rate_id']>0){
											$parameter1 .= (($value['primary_rate']) ? '<span class="ashstrick">*</span> ' : '').'<span>@'.$value['goal_rate'].' '.Model::instance('Model/workouts')->getGoalValues('rate',$value['goal_rate_id']).'</span> ';
										}
										...
										if($value['goal_angle']>0 && $value['goal_angle_id']>0){
											$parameter1 .= (($value['primary_angle']) ? '<span class="ashstrick">*</span> ' : '').'<span>'.$value['goal_angle'].'%'.Model::instance('Model/workouts')->getGoalValues('angle',$value['goal_angle_id']).'</span> ';
										}
										...
										if($intvalue>0){
											$parameter1 .= (($value['primary_int']) ? '<span class="ashstrick">*</span> ' : '').'<span>'.$intvalue.' int</span>';
										}
										...
										if($value['goal_rest_mm'] + $value['goal_rest_ss']>0){
											if($value['goal_rest_mm'] >0 ||  $value['goal_rest_ss']>0){
												$parameter1 .=  (($value['primary_rest']) ? '<span class="ashstrick">*</span> ' : '').' <span>'.$value['goal_rest_mm'];
										...
										$parameter1 .= '</a>';
										echo (trim(strip_tags($parameter)) != '' ? (trim(str_replace(strip_tags($parameter1),'&nbsp;','')) != '' ? $parameter.' / '.$parameter1 : $parameter) : $parameter1);
									?>
									</div>
...
?>
	...
	<input type="hidden" id="wkout_id" name="wkout_id" value="<?php echo (isset($wkout_id) ? $wkout_id : $wkout_id)?>"/>
	...
	<div class="row">
		<div class="border">
			...
			<a data-ajax='false' data-onclick="$('#save_edit').val(5);$('#form-workoutrec').submit();" data-text="<?php echo __('Clicking BACK or CANCEL will discard any changes. Clicking MORE will display record options such as SAVE. Continue with exiting?');?>" href="javascript:void(0);" class="btn btn-default confirm" data-allow="<?php echo (Helper_Common::getAllowAllAccessByUser($session->get('user_allow_page'),'is_confirm_wkout_hidden') ? 'false' : 'true');?>" data-notename="hide_confirm_wkout"><?php echo (!empty($popupAct) ? __('cancel') : __('back') ); ?></a></div>
			<div class="col-xs-3 aligncenter save-icon-button">
				<i class="fa fa-ellipsis-h iconsize" onclick="getTemplateOfWorkoutAction();"></i>
			</div>
		</div>
