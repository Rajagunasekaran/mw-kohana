...
<div id="page-wrapper">
	<div class="container-fluid">
		...
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header">Edit <?php echo (isset($_GET['d']) && $_GET['d']==1 ? 'Default' : 'Sample');?> Workout Records</h1>
				<ol class="breadcrumb">
					<li><i class="fa fa-dashboard"></i>  <a href="<?php echo URL::base().'admin/index'; ?>">Dashboard</a></li>
					<li class="active"><i class="fa fa-edit"></i> Edit <?php echo (isset($_GET['d']) && $_GET['d']==1 ? 'Default' : 'Sample');?> Workout Records</li>
				</ol>
         </div>
		</div>
      <!-- /.row -->
<?php /*********************************************** Edit Section **************************************************/ ?>
	...
	<?php if(isset($_GET['d']) && $_GET['d']==1){ ?><input type='hidden' name='d' value='<?php echo $_GET['d']; ?>'><?php } ?>
	<input type="hidden" value="<?php echo $save;?>" name="save_edit" id="save_edit"/>
	<input type="hidden" value="0" name="parent_folder_id"/>
	...
	<div class="row">
		<div class="border">
			<div class="col-xs-3 aligncenter">
				<a data-ajax='false' data-text="<?php echo __('Clicking BACK or CANCEL will discard any changes. Clicking MORE will display record options such as SAVE. Continue with exiting?');?>" href="<?php echo URL::base().'admin/workout/sample/'.(isset($_GET['d']) ? '?d='.$_GET['d'] : ''); ?>" class="triangle confirm" data-notename="hide_confirm_wkout" data-allow="<?php echo (Helper_Common::getAllowAllAccessByUser($session->get('user_allow_page'),'is_confirm_wkout_hidden') ? 'false' : 'true');?>">
					...
					<div class="col-xs-9 activedatacol alignleft">
						<div class="pointers colormodelpopup">
							<div class="col-xs-8 alignleft colormodelpopup" style="padding:0px">
								<input data-ajax="false" data-role="none" type="text" id="wrkoutname_hidden" class="wrkoutname_hidden input-sm form-control" name="wrkoutname_hidden" placeholder="click to Enter custom title" value="<?php echo ucfirst($workoutRecord['wkout_title']);?>"/>
							</div>
...
?>
	...
	<div class="row">
		<div class="border">
			...
			<a data-ajax='false' data-text="<?php echo __('Clicking BACK or CANCEL will discard any changes. Clicking MORE will display record options such as SAVE. Continue with exiting?');?>" href="<?php echo URL::base().'admin/workout/sample'.(isset($_GET['d']) ? '?d='.$_GET['d'] : ''); ?>" class="btn btn-default confirm" data-notename="hide_confirm_wkout" data-allow="<?php echo (Helper_Common::getAllowAllAccessByUser($session->get('user_allow_page'),'is_confirm_wkout_hidden') ? 'false' : 'true');?>"><?php echo (!empty($popupAct) ? __('cancel') : __('back') ); ?></a></div>
			<div class="col-xs-3 aligncenter save-icon-button">
				<i class="fa fa-ellipsis-h iconsize" onclick="getTemplateOfWorkoutAction();"></i>
			</div>
		</div>
