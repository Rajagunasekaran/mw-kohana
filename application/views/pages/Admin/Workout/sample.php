      ...
      <div id="page-wrapper">
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <h1 class="page-header">
										...
										<?php
										if($default_status==0){
											echo (isset($site_language['Sample Workout Records'])) ? $site_language['Sample Workout Records'] : 'Sample Workout Records';
										}else{
                        ...
                        <ol class="breadcrumb">
                            ...
                            <li class="active">
                                <i class="fa fa-edit"></i> <?php echo ($default_status==0)?"Workout Sample Records List":"Workout Default Records List"; ?>
                            </li>
                        </ol>
                    </div>
                </div>
				...
				<div class="gallery-div">
					...
					<?php $src_url = URL::base().'admin/workout/sample';
						if($default_status==1){
								...
								<div class="col-sm-6"  >
									...
									if(Helper_Common::hasAccess('Manage Workouts')) { ?>
											<select name="right moteactions[]" id="moteactions" class="moteactions selectAction">
												...
												<option disabled value="deletesampleworkouts">Delete selected </option>                                                
												<!--option value="tagworkouts">Tag Selected Workouts</option-->
												<?php if($default_status==0){?>
														<!--option value="reportEmail1">Email</option-->
														<?php if (Helper_Common::is_admin()){
																echo '<option value="sampleDefault">Copy selected to Default</option>';
														}
												} ?>
                           ...
                           <tbody id="table-content-contnr">
                        	<?php foreach($workout_details as $key => $value) { ?>
                            	<tr id="row-<?php echo $value['wkout_id'];?>">
			                   		<td class="tabl-chkbox"><input type="checkbox" name="wkoutselect[]" id="wkoutselect" class="wkoutselect" value="<?php echo $value['wkout_id'];?>" />
			                   		</td>
                   					<td align="center"><div class="colorcode <?php echo strtolower($value['color_title']); ?>"></div></td>
                   					<td><a onclick="getworkoutpreview('<?php echo $value['wkout_id'];?>')" href="javascript:void(0);"><?php echo $value['wkout_title'];?></a>
                   					</td>
                   					<td>
			                   			<?php
                   					...
                   					<td>
		                   				...
		                   				<select name="wkoutaction[]" id="<?php echo $value['wkout_id'];?>" class="wkoutaction selectAction" onchange="goto_action('<?php echo $value['wkout_id'];?>','<?php echo $value['folder_id'];?>',this.value)">
		                   					<option value="">Select an Option</option>
		                   					<?php if($value['default_status'] == '0' || Helper_Common::is_admin()) { ?>
	                   						<option value="sampleedit">Edit</option>
	                   						<?php } ?>
	                   						<?php if($value['default_status'] == '1' && (Helper_Common::is_manager() || Helper_Common::is_trainer())) { ?>
                   							<option value="sampledefaulthide">Hide</option>
                   							<?php } ?>
                   							<?php if($default_status==0){?>
                   							<option value="sample duplicate">Duplicate</option>
                   							<?php } ?>
                   							<?php if($default_status==1 && Helper_Common::is_admin()) { ?>
                								<option value="default duplicate">Duplicate</option>
                								<?php } ?>
                								<option value="Preview">Preview</option>
                								<?php if($value['default_status'] == '0') { ?>
                								<option value="deletesample">Delete</option>
                								<?php }elseif($default_status==1 && Helper_Common::is_admin()) { ?>
                								<option value="deletedefault">Delete</option>
                								<?php } ?>
                								<?php if($default_status==0){?>
                								<?php if(Helper_Common::is_admin()) { ?>
             									<option value="sampleDefault">Copy to Default</option>
             									<?php } ?>
             									<option value="share">Share</option>
             									<option value="sampleexport">Export</option>
             									<?php } ?>
             									<!--option value="tag">Tag</option-->
             									<!--option value="sample_more">More</option-->
             								</select>
             							</td> 
          								<?php } ?>
          							</tr>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
				...
				if($default_status==0){
						...
						<div class="form-group">
							<div class="col-xs-12 navimagedetails">
								<div class="navimgdet1"><b><input type='radio' id="wktype2" name='workout_type' onclick='$("#workout_type").val("sampleToworkout");'> <label for="wktype2" >Duplicate into Workout plans</label></b></div>
							</div>
						</div>
						<?php
				}
						...
						<div class="form-group">
							<div class="col-xs-12 navimagedetails">
								<div class="navimgdet1"><b><input type='radio' id="wktype1" name='workout_type' onclick='$("#workout_type").val("sample");' checked='checked'><label for="wktype1" > Duplicate into Default Workouts</label></b></div>
							</div>
						</div>
						<div class="form-group">
							<div class="col-xs-12 navimagedetails">
								<div class="navimgdet1"><b><input type='radio' id="wktype2" name='workout_type' onclick='$("#workout_type").val("defaultToworkout");'> <label for="wktype2" >Duplicate into Workout plans</label></b></div>
							</div>
						</div>
						<?php
				}
...
<div data-backdrop="static" data-keyboard="false" role="dialog" class="modal fade" id="listwkouts" style="display: none;">
	<div class="vertical-alignment-helper">
         <div class="modal-dialog">
            <div class="modal-content">
...
<!-- Set As Default -->
<input type='hidden' id='wkt_type' value='sample'>
<input type="hidden" id="newlyAddedXr" name="newlyAddedXr" value="0"/>
<input type='hidden' id='default_type' value='<?php echo $default_status; ?>'>
<!-- Export -->
