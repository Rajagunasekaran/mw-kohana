		...
		<div id="page-wrapper">
			<div class="container-fluid pageexebrowse">
			 	...
			 	<div class="row">
				  	<div class="col-lg-12">
						...
						if($session->get('current_site_name')!=''){echo '"'.$session->get('current_site_name').'"';}?>
							<?php echo ($default_status==3) ? __("Shared Exercise Record(s)") : __("My Exercise Record(s)"); ?>
						</h1>
						<ol class="breadcrumb">
						 	<li>
						 	...
						 	<li class="active">
						 		<i class="fa fa-edit"></i> <?php echo __('Exercise Records List'); ?>
							</li>
						</ol>
				  	</div>
 				</div>
				...
				<div class="row">
					<h2 class="col-lg-6">
						<?php if(isset($template_details_all) && count($template_details_all)>0) { echo count($template_details_all); }?> 
						 <?php echo __('Exercise Record(s)'); ?></h2>
					<div class="col-lg-6"> 
					<!--
				...
				<div class="gallery-div">
					<form class="advnce-srch-frm" action="<?php echo URL::base().'admin/exercise/browse'.($default_status==3 ? '?d=3' : '');?>" method="post">
						<div class="topsearch">
							<div class="border">
								<div class="col-xs-11">
							...
							<div class="col-sm-10 filter_exe">
									<label for="sortby"><?php echo __('Sort By'); ?>Sort By :</label>
				...
				<?php if(isset($template_details) && count($template_details)>0) { ?>
						...
						<div class="topsearch topsearch_white">
							<div class="border" >
								<div class="col-sm-6 showentries">
									...
									<form class="" name='show' id='show' method="get" action="<?php echo URL::base().'admin/exercise/browse/';?>">
								...
								<?php if(Helper_Common::hasAccess('Manage Exercise')) { ?>
									<select name="moteactions[]" id="moteactions" class="form-control selectAction right exerciseaction" >
										...
										<?php if(Helper_Common::hasAccess('Create Exercise')) { ?>
											<option value="create"><?=__("Create New Exercise Record")?></option>
										<?php } ?>
										...
										<?php if(Helper_Common::is_admin() || Helper_Common::is_manager()){ ?>
										...
										<?php if($default_status!=3){
										 	if(Helper_Common::is_admin() || Helper_Common::is_manager() || Helper_Common::is_trainer()){ ?>
												<option value="sample"><?=__("Copy selected to Sample")?></option>
					...
					<?php if(Helper_Common::hasAccess('Manage Exercise')) { ?>
						<select name="moteactions[]" id="moteactions" class="form-control selectActions right exerciseaction" >
							<option value="">Actions</option>
							<?php if(Helper_Common::hasAccess('Create Exercise')) { ?>
								<option value="">Create New Exercise Record</option>
							<?php } ?>
							...
							<option value="share">Share this record </option>
							<option value="feedback">Feedback for this record</option>
							<option value="reportEmail">Email</option>
							<option value="Excel">Excel</option>
							<option value="PDF">PDF</option>
									 ...
									 <div class="col-lg-12">
										...
										foreach($template_details as $key => $value) { ?>
										 <tr id="row-<?php echo $value['id'];?>">
											...
											<?php if($default_status==3){ ?>
												<td class="click-prev" style="text-align: left;">
													<div class="col-xs-3" style="padding-left: 0; text-align: center;"><img src="<?php echo $value['featimg'];?>" width="50"/></div>
													<div class="col-xs-9"><span class="ex_name highlight break-xr-name" style="font-weight: 600;"><?php echo $value['name']; ?></span></div>
												</td>
											  	<td><span class="highlight" style="font-weight: 600;"><?php echo $value['user_name']; ?></span><br><span class="item-info-bold"><?php echo $value['shared_by']; ?></span></td>
												<td><?php echo $value['shared_msg']; ?></td>
											<?php }else{ ?>
												<td class="click-prev">
													<img src="<?php echo $value['featimg'];?>" width="50"/>
												</td>
											...
											<?php } if(Helper_Common::hasAccess('Manage Exercise')) { ?>
												<td>
													<select id="<?php echo $value['id'];?>" name="exerciseaction" class="exeselect selectAction ex-single-action form-control">
														...
														//if($default_status!=3){
															if(Helper_Common::is_admin() || Helper_Common::is_manager() || Helper_Common::is_trainer()){ ?>
																<option value="sample"><?=__("Copy to Sample")?></option>
															<?php } if(Helper_Common::is_admin()){ ?>
			...
			<div class="modal-content">
				<div class="modal-header">
					<div class="row">
						<div class="popup-title">
							...
							<div class="col-xs-8 ratetitle"><?php echo __('Options for this Exercise Export'); ?></div>
				...
				<div class="modal-body ratebody">
					...
					<div class="row opt-row-detail">
						<div class="col-xs-12">
							<a onclick="$('#exportModal').modal('hide');listexport('excel', <?php echo ($default_status==3) ? 3 : 0; ?>);" class="btn btn-default"
					...
					<div class="row opt-row-detail">
						<div class="col-xs-12">
							<a class="btn btn-default" style="width:100%" onclick="$('#exportModal').modal('hide');listexport('pdf', <?php echo ($default_status==3) ? 3 : 0; ?>)" href="javascript:void(0)" >
			...
			<div class="modal-content">
				<form name="email_rep" id="email_report_frm" onsubmit="return exercise_email()">
					...
					<div class="modal-body">
						...
						<input type="hidden" name="d_status" id="d_status" value="0"/>
...
<input type='hidden' name='xrdefault' id='xrdefault' value='0'>

<!-- Set as default -->
<div class="modal fade" id="setexesampleModal" tabindex="-1" role="dialog" aria-labelledby="setexesampleModal">
	<div class="vertical-alignment-helper">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					...
					<div class="col-xs-6"><?php echo __('Set Sample Exercise Record'); ?></div>
					<div class="col-xs-3 save-icon-button bluecol">
					<button class="btn" style="background-color:#fff" name="f_method" onclick="set_default_submit();" data-ajax="false" type="button" data-role="none">
					<i class="fa fa-check-square-o" data-toggle="collapse" style="font-size:30px;"></i>
					</button>
