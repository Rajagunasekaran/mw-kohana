...
function getexercisesetTemplateAjaxEdit(elem, goalOrder, type) {
    ...
    var wkoutId = $('#wkout_assign_id').val();
...
function editExercise(selector, order) {
    ...
    setTimeout(function () {
        if (newUnitIdbefore != newUnitId && ($.isNumeric(newUnitIdbefore) || $.isNumeric(newUnitId))) {
            ...
            var wkout_id = $('#wkout_assign_id').val();
            var selectornew = 'li#' + selector;
            var inlineString = $(selectornew).html();
            if (newUnitId == 0) {
                newUnitId = '0_' + $('#wkout_assign_id').val();
            }
...
function getTemplateOfExerciseSetActionByprev(exerciseSetId, link, goalOrder) {
    ...
    if (exerciseSetId.indexOf('new') >= 0)
        ...
        titlediv = 'div#itemset_' + $('#wkout_assign_id').val() + '_' + exerciseSetId + ' div.navimgdet1 b';
    $('#myOptionsModalAjax').html();
    $.ajax({
        url: siteUrl + "search/getmodelTemplate",
        data: {
...
function checkallItems(selector) {
    if ($(selector).hasClass('checked')) {
        $("input.checkhidden:checkbox").prop('checked', false);
...
function getTemplateOfAssignAction(wkoutId, wkoutAssignId, markedState) {
    $('#myModal').html();
    $.ajax({
        url: siteUrl + "search/getmodelTemplate",
        data: {
...
function ChangeMarkStatus(status, goalId) {
    $('#markedstatus_' + goalId).val(status);
    if (status == '1') {
        $('#markstatus_' + goalId).removeAttr('class').attr('class', 'fa fa-check-square-o iconsize greenicon listoptionpop');
    } else {
        $('#markstatus_' + goalId).removeAttr('class').attr('class', 'fa fa-minus-square-o iconsize pinkicon listoptionpop');
    }
    closeModelwindow('');
    return true;
}
function addReAssignWorkouts(wkoutAssignid, wkoutId, date) {
    $('#myOptionsModal').html();
    $.ajax({
        url: siteUrl + "search/getmodelTemplate",
        data: {
...
function addAssignWorkouts(wkoutAssignid, wkoutId, date) {
    $('#myOptionsModal').html();
    var date = $('input#assigned_date').val();
    $.ajax({
        url: siteUrl + "search/getmodelTemplate",
        data: {
            action: 'addAssignWorkouts',
...
function addDatetoAssign(date, ModelType) {
    ...
    $(formdata).each(function (i, field) {
        if (field.name == 'selected_date') {
            $('#assigned_date_hidden_text').html('<b>' + field.value + '</b>');
            if ($('#assigned_date_hidden'))
                $('#assigned_date_hidden').val(field.value);
            if ($('#assigned_date'))
                $('#assigned_date').val(field.value);
        }
    });
