...
function sendByEmail(alldetailinput) {
	...
	$.post(siteUrl + "export/generateWkoutTemp", {
...
function insertToWkout(fid, type, method, date) {
	...
	if (type == 'wrkout')
		href = typeXr + 'startwkout/' + fid + '?act=edit&fid=' + folder_id;
	else if (type == 'sample')
		href = typeXr + 'startsample/' + fid + '?act=edit&fid=' + folder_id;
	else if (type == 'shared')
		href = typeXr + 'startshare/' + fid + '?act=edit&fid=' + folder_id;
	else if (type == 'assigned')
		href = typeXr + 'startassign/' + fid + '?act=edit&fid=' + folder_id;
	else if (type == 'wkoutlog')
		href = typeXr + 'startwklog/' + fid + '?act=edit&fid=' + folder_id;
...
if (allowTour) {
	...
	(function () {
		var tour = new Tour({
			storage: false,
			template: "<div class='popover tour'><div class='arrow'></div><h3 class='popover-title'></h3><div class='popover-content'></div><nav class='popover-navigation'><div class='btn-group'><button class='btn btn-sm btn-default' data-role='prev' data-ajax='false'>Prev</button><button class='btn btn-sm btn-default' data-role='next' data-ajax='false'>Next</button></div><button class='btn btn-sm btn-default btn-end' data-role='end' id='closetour' data-ajax='false'>End tour</button></nav><div class='popover-content-custom'><input type='hidden' name='page_id' id='page_id' value='3'/><input onclick='notifyUpdate(this);' type='checkbox' name='hide_tour' value='1' id='hide_tour' data-ajax='false' data-role='none'/> <label for='hide_tour'>Don't show this dialog again</label></div></div>",
			onEnd: function () {}
		});
		tour.addSteps([
			{
				...
				content: "Create a new folder for workout plans."
			}, {
				...
				placement: "bottom",
				title: "Options for wokout plan",
				content: "Choose the workoutplans and do the actions."
			}, {
				element: ".tour-step.tour-step-4",
				placement: "top",
				title: "Preview Workout Plan",
				content: "Click the Workout Plan and preview the record and do the actions."
			}, {
				element: ".tour-step.tour-step-5",
				placement: "top",
				title: "Options for the Workout Plan",
				content: "Click the '...' and open the options modal and do the actions."
			}
		]);
