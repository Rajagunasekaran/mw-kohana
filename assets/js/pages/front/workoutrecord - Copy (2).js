	...
	$( "ul#sTree2 li" ).each(function(i, lival){
		...
		if(positions[keyval] != undefined && $.inArray(i,positions[keyval]) !='-1' && $.isNumeric( $( "ul#sTree2 li" ).eq(i).attr('data-id') )){
			...
			for(var keynew =1 ; keynew < positions[keytitle].length ; keynew++){
				$( "ul#sTree2 li" ).eq(positions[keytitle][keynew])
				$( "ul#sTree2 li" ).eq(positions[keytitle][0]).find('.exercisesetdiv').append('<hr>');
				curele = $( "ul#sTree2 li" ).eq(positions[keytitle][keynew]).find('.exercisesetdiv').html();
				newOrder = $( "ul#sTree2 li" ).eq(positions[keytitle][0]).find('input.seq_order_combine_up').val();
				oldOrder = $( "ul#sTree2 li" ).eq(positions[keytitle][keynew]).find('input.seq_order_combine_up').val();
				var re5 = new RegExp(oldOrder + '_', 'g');
				curele = curele.replace(re5, newOrder+'_');
				$( "ul#sTree2 li" ).eq(positions[keytitle][0]).find('.exercisesetdiv').append(curele);
				$( "ul#sTree2 li" ).eq(positions[keytitle][keynew]).remove();
			}
...
function editWorkoutRecord(elem, method, setId) {
   ...
   if ($('li#itemSet_' + wkoutId + '_' + elem).length && $('div#itemset_' + elem).length) {
      var goal_id = elem;
      var img_url = '';
      if ($('div#itemset_' + elem + " .activelink").attr("disabled") && methodpreview) return false;
   ...
   } else if ($('div#itemset_' + elem).length && method == "create") {
      var goal_id = img_url = '';
      if ($('div#itemset_' + elem + " .activelink").attr("disabled") && methodpreview) return false;
      var goalOrder = $('div#itemset_' + elem + ' input#goal_order_' + elem).val();
      var dataForm = $('div#itemset_' + elem + ' input').map(function() {
					  return {
						name: $(this).attr('name'),
						value: $(this).attr('value'),
						keyval: $(this).attr('data-keyval')
      ...
      img_url = '';
      if ($('div#itemset_' + elem + ' .navimage img').length) {
         if ($('div#itemset_' + elem + ' .navimage img').attr('src') != '') {
            var img_url = $('div#itemset_' + elem + ' .navimage img').attr('src').replace(siteUrl, '');
            img_url = img_url.replace('../../../', '');
         }
      }
...
function addnewExercise(elem) {
      ...
      setTimeout(function() {
         ...
         if ($('li#itemSet_' + elem + ' div#set_id_' + field.keyval + ' a.' + newvariable + '_div').length && $('#createExercise div#set_' + field.keyval + ' span.' + newvariable).length) {
            ...
            if (updatedText != '<span class="inactivedatacol">Click to modify</span>') {
               ...
               else $('li#itemSet_' + elem + ' div#set_id_' + field.keyval + ' a.' + newvariable + '_div').html(updatedText);
            } else {
   ...
   setTimeout(function() {
	   ...
	   if($.isNumeric(newUnitId))
		   $(selectornew + ' .navimage').attr("onclick", "getTemplateOfExerciseRecordAction('" + order + '_'+ newUnitId + "',this,'"+order+"');");
	   $(selectornew).attr("data-id",newUnitId);
	   $(selectornew).attr("id",'itemSet_' + elem.replace(oldUnitId, order+'_'+newUnitId));
		updateLiItems();
	}, 170);
   return false;
};
...
function doDeleteProcess(type, selector, targetId) {
   ...
   if (type == 'exerciseset') {
      if (confirm('Deleting this Exercise Set will not be saved until all updates to the Workout Plan have been confirmed.')) {
         if ($('li#itemSet_' + wkoutid + '_' + selector+' div.exercisesetdiv div#set_id_'+targetId).length) {
            ...
            $('li#itemSet_' + wkoutid + '_' + selector+' div.exercisesetdiv div#set_id_'+targetId).remove();
			...
			inputUpdateVal = $('input#itemSet_' + wkoutid + '_' + selector+'_hidden').val();
			var inputUpdateVal = removeCommaWithValue(inputUpdateVal,targetId);
			$('input#itemSet_' + wkoutid + '_' + selector+'_hidden').val(inputUpdateVal);
			lengthChild =$('div#itemset_'+selector+' div.exercisesetdiv').children().length;
			oldTag = '';
			$('div#itemset_'+selector+' div.exercisesetdiv').children().each(function(i,item){
				hrtag = $(item).prop("tagName");
				if((i==0 && hrtag=='HR') || (i==lengthChild - 1 && hrtag=='HR') || (oldTag == hrtag && hrtag=='HR' && i >0 && i < lengthChild - 1)){
					$(this).remove();
					if(oldTag == hrtag && hrtag=='HR' && i >0 && i < lengthChild - 1)
						oldTag = '';
				}else{
					oldTag = hrtag;
				}
			});
			if($('div#itemset_'+selector+' div.exercisesetdiv div.navimgdet2').length==0){
