...
$(document).ready(function () {
	$('.tt').on('click',function (e) {
		...
		if (s.length>1) {
			...
			$.ajax({
			url : siteUrl+"commonquestions/questionupdates",
				cache: false,
				type: "POST",
				data : {
					id : s[0],
...
$('#tabledrop').sortable({
	...
	stop: function(event, ui) {
		...
		//alert(data)
		$.ajax({
			url : siteUrl+"commonquestions/changequestionorder",
			cache: false,
			type: "POST",
			data : {
				data : data
...
function goto_qaction(sqid, arg) {
	...
	if (arg == 'delete') {
      ...
      if (r) {
         $.ajax({
            url: siteUrl + "commonquestions/removequestion",
            method: 'post',
            data: {
               id: sqid
            },
		...
		$.ajax({
			url: siteUrl + "commonquestions/getquestionoption",
			method: 'post',
			data: {
				id: sqid
			},
				...
				if (content) {
					...
					$('#sTreequestion').sortable({
						...
						stop: function(event, ui) {
							...
							//alert(data)
							$.ajax({
								url : siteUrl+"commonquestions/changeorder",
								cache: false,
								type: "POST",
								data : {
									id : sqid,
...
function addquestions(){
	...
	$.ajax({
		url: siteUrl + "commonquestions/add_edit_questions",
		method: 'post',
		data: {
			question: q,
			isreq:isreq,
...
function addquestionoptions(){
	...
	$('#option').val('');
	$.ajax({
		url: siteUrl + "commonquestions/add_questionoptions",
		method: 'post',
		data: {
			sqid: sqid,
			option: option,
...
function previewall(){
	...
	$.ajax({
		url: siteUrl + "commonquestions/getallquestionoption",
		method: 'post',
		data: {},
		success: function(content) {
			if (content) {
...
function deleteoption(sqid,optid,wseq){
	...
	if (r) {
		...
		$.ajax({
			url: siteUrl + "commonquestions/removequestionoption",
			method: 'post',
			data: {
				id: sqid,
				oid: optid
...
function updateOption(oid,option){
	var sqid = $('#sqid').val();
	$.ajax({
		url: siteUrl + "commonquestions/update_questionoptions",
		method: 'post',
		data: {
			id: oid,
			option: option,
